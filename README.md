...
* Run `Abp.Acme.DbMigrator` to create the initial database. This step is also automatically done when you create a new solution, if you didn't especially disabled it. This should be done in the first run. It is also needed if a new database migration is added to the solution later.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p 89462093-b2fb-45e9-837d-b7a623ffaea0
```

> `89462093-b2fb-45e9-837d-b7a623ffaea0` is the password of the certificate, you can change it to any password you want.
...
* `Abp.Acme.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
